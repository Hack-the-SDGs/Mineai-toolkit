...
const TOOL_DESC = {
  ...
  pathfinder_goto_near: "阻塞直到 bot 抵達 ``(x, y, z)`` 的 ``radius`` 範圍內；用於「移動到某處」，非對準要互動的方塊。",
  pathfinder_goto_look_at_block: "阻塞直到 bot 進入可及範圍並面向目標方塊；要挖掘／使用／放置某個方塊前用這個。",
  pathfinder_check_path: "只規劃路徑但不移動，回報目標是否可抵達（reachable、status、path 等）；在 goto 前先用它驗證。",
